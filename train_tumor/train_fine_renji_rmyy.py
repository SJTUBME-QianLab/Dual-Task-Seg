...
from data.dataloader import Generalize_Tumor_Skeleton_in_Tumor_Renji_RMYY
...
def get_args():
    ...
    parser.add_argument(
        ...
        default=800,
        metavar="N",
        help="number of epochs to train_tumor (default: 300)",
    )
    parser.add_argument(
        "--size",
        type=int,
        default=6000000,
        metavar="N",
        help="number of size (default: 4000000)",
    )
    parser.add_argument(
        "--lr",
        type=float,
    ...
    parser.add_argument(
        "--workers", type=int, default=8, metavar="N", help="num_worker (default: 0)"
    )
    parser.add_argument("--log", action="store_false", help="write logs or not")
    parser.add_argument("--weight_std", action="store_false", help="write logs or not")
...
def generalize_train_pool_u_conresnet(args):
    ...
    model = U_CorResNet_Fix_Contrastive_Proj_DT_CCLS_RC(weight_std=args.weight_std).train()
    ...
    dset_train = Generalize_Tumor_Skeleton_in_Tumor_Renji_RMYY(
        data=args.data, augment=True, sdf=True, size=args.size
    )
    ...
    print("weight_std:", args.weight_std)
    print("#################################")
...
if __name__ == "__main__":
    ...
    if args.time == "time":
        args.time = get_time()
    args.time = "Tumor_3D_Fine_Fine_On_%s_%s" % (
        args.data,
        args.time,
    )
