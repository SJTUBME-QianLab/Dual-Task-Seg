...
def get_args():
    ...
    parser.add_argument(
        "--workers", type=int, default=16, metavar="N", help="num_worker (default: 16)"
    )
...
def generalize_train_pool_u_conresnet(args):
    ...
    dset_train = Generalize_Tumor_Skeleton(
        data=args.data, augment=True, sdf=True, size=4000000
    )
    ...
    for i in range(args.epochs):
        ...
        if (i + 1) % 10 == 0 or i >= (args.epochs - 10):
            torch.save(
                ...
                join(checkpoint_path, "Tumor_3D_Fin_%s.pth" % args.data),
            )
