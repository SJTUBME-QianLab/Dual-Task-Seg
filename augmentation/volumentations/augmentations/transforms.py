...
from ..core.transforms_interface import Transform, DualTransform, TripleTransform
...
class Resize(DualTransform):
    def __init__(self, interpolation=3, always_apply=False, p=1):
        super().__init__(always_apply, p)
        self.interpolation = interpolation

    def get_params(self, **data):
        return {"shape": data["size"]}

    def apply(self, img, shape):
        return F.resize(img, new_shape=shape, interpolation=self.interpolation)

    def apply_to_mask(self, mask, shape):
        return F.resize(mask, new_shape=shape, interpolation=0)


class Resize3(TripleTransform):
    def __init__(self, interpolation=3, always_apply=False, p=1):
        super().__init__(always_apply, p)
        self.interpolation = interpolation
    ...
    def apply_to_coarse(self, coarse, shape):
        return F.resize(coarse, new_shape=shape, interpolation=0)
...
class RandomScale3(TripleTransform):
    def __init__(
        self, scale_limit=[0.9, 1.1], interpolation=3, always_apply=False, p=0.5
    ):
        super().__init__(always_apply, p)
    ...
    def apply_to_coarse(self, coarse, scale):
        return F.rescale(coarse, scale, interpolation=0)
...
class RandomFlip3(TripleTransform):
    def __init__(self, axis=0, always_apply=False, p=0.5):
        super().__init__(always_apply, p)
        self.axis = axis
...
class ElasticTransform(DualTransform):
    ...
    def get_params(self, **data):
        ...
        return {"random_state": random_state, "sigmas": sigmas, "alphas": alphas}


class ElasticTransform3(TripleTransform):
    def __init__(
        self,
        deformation_limits=(0, 0.25),
        interpolation=3,
    ...
    def apply_to_coarse(self, img, sigmas, alphas, random_state=None):
        return F.elastic_transform(
            img,
            sigmas,
            alphas,
...
class RandomRotate3(TripleTransform):
    def __init__(
        self,
        x_limit=(-15, 15),
        y_limit=(-15, 15),
    ...
    def apply_to_coarse(self, coarse, x, y, z):
        return F.rotate3d(
            coarse,
            x,
            y,
