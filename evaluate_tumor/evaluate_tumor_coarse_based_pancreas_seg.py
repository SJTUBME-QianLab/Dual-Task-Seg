...
def evaluate_on_test_list(args):
    ...
    for i in range(len(args.test_list)):
        ...
        image, _, bbox = crop_by_pancreas(images, mask, coarse, MARGIN=30, MARGIN_Z=8)
        mask = mask == 2
            ...
            if not os.path.exists(
                    join(
                        ...
                        "pred_{}_{}_{}_based_pancreas_segmentation".format(
                            args.data, args.ensemble, args.thresh
                        ),
                    )
            ):
                os.mkdir(
...
if __name__ == "__main__":
    ...
    args.log_file = (
            "cal_time_ensemble_%s_on_%s_%s_All_based_pancreas_segmentation.txt"
            % (args.ensemble, args.data, args.thresh)
    )
