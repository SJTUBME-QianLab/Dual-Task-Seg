...
def get_args():
    ...
    parser.add_argument(
        "--coarse_path",
        type=str,
        default="Tumor_3D_Patch_CL_DT_In_Pancreas_On_renji,rmyy_2022_11_15_17_21",
        metavar="N",
...
def get_data_mask_coarse(args, index, use_coarse=True):
    ...
    HAS_FILE = os.path.exists(
        join(
            args.coarse_path,
            "pred_{}_{}_{}".format(args.data, args.coarse_ensemble, args.coarse_thresh),
            "{}.nrrd".format(index),
        )
    )
...
def evaluate_on_test_list(args):
    ...
    for i in range(len(args.test_list)):
        try:
            ...
            images, mask, coarse = get_data_mask_coarse(
                args, args.test_list[i], use_coarse=True
            )
            mask[mask != 2] = 0
            mask[mask == 2] = 1
            coarse[coarse > 0] = 1
...
if __name__ == "__main__":
    ...
    args.log_file = "save_true_%s_%s_on_%s_based_pred_tumors_%s_%s.txt" % (
        args.ensemble,
        args.thresh,
        args.data,
        args.coarse_thresh,
